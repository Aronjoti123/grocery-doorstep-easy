...
import { ORDER_STATUS_LABELS, OrderStatus, Category, CATEGORY_LABELS } from '@/types/grocery';
...
const AdminPage = () => {
  const { isAdmin, orders, updateOrderStatus, refreshOrders, products, refreshProducts, loading, authUser } = useApp();
  ...
  useEffect(() => {
    if (isAdmin) { refreshOrders(); refreshProducts(); }
  }, [isAdmin]);

  if (loading) return <div className="text-center py-12 text-muted-foreground">Loading...</div>;
  if (!authUser || !isAdmin) return <Navigate to="/login" replace />;

  const handleAddProduct = async () => {
    if (!newName || !newPrice || !newQuantity) { toast.error('Fill all fields'); return; }
        ...
        <TabsContent value="products" className="space-y-2">
          {products.map(p => (
            <div key={p.id} className="bg-card rounded-lg p-3 card-elevated flex items-center justify-between">
              ...
              <Button variant={p.inStock ? 'outline' : 'destructive'} size="sm" className="text-xs"
